        ...
        static public void SaveAscii(List<STLTriangle> triangles, string fileName)
        {
            ...
            if (triangles.Count > 0)
            {
                ...
                foreach (STLTriangle tri in triangles)
                {
                    ...
                    file.Add("outer loop");
                    file.Add("vertex " + tri.Vert[0].X.ToString("f7") + " " +
                        tri.Vert[0].Y.ToString("f7") + " " + tri.Vert[0].Z.ToString("f7"));
                    file.Add("vertex " + tri.Vert[1].X.ToString("f7") + " " +
                        tri.Vert[1].Y.ToString("f7") + " " + tri.Vert[1].Z.ToString("f7"));
                    file.Add("vertex " + tri.Vert[2].X.ToString("f7") + " " +
                         tri.Vert[2].Y.ToString("f7") + " " + tri.Vert[2].Z.ToString("f7"));
                    file.Add("end loop");
                    file.Add("endfacet");
                }
                file.Add("end solid");
            ...
            try
            {
                ...
                using (System.IO.BinaryWriter bw = new System.IO.BinaryWriter(System.IO.File.Open(fileName, System.IO.FileMode.Create)))
                {
                    ...
                    foreach (STLTriangle tri in triangles)
                    {
                        ...
                        bw.Write((float)tri.Vert[0].X);
                        bw.Write((float)tri.Vert[0].Y);
                        bw.Write((float)tri.Vert[0].Z);
                        bw.Write((float)tri.Vert[1].X);
                        bw.Write((float)tri.Vert[1].Y);
                        bw.Write((float)tri.Vert[1].Z);
                        bw.Write((float)tri.Vert[2].X);
                        bw.Write((float)tri.Vert[2].Y);
                        bw.Write((float)tri.Vert[2].Z);
                        bw.Write((UInt16)0);
